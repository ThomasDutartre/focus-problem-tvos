...
xcodebuildmcp device launch --device-id DEVICE_UDID --bundle-id com.example.MyApp --app-path /path/to/MyApp.app
```

### Capture Logs On Simulator

```bash
xcodebuildmcp logging start-simulator-log-capture --simulator-id SIMULATOR_UDID --bundle-id com.example.MyApp
...
```bash
xcodebuildmcp simulator launch-app --bundle-id com.example.MyApp --simulator-id SIMULATOR_UDID
```
...
```bash
xcodebuildmcp debugging attach --bundle-id com.example.MyApp --simulator-id SIMULATOR_UDID
```
